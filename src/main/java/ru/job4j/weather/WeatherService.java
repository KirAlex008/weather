...
public class WeatherService {
    ...
    public Mono<Weather> findMax() {
        Optional<Weather> el = weathers.values()
                .stream()
                .max(Comparator.comparing(Weather::getTemperature));
        return Mono.justOrEmpty(el);
    }

    public Flux<Weather> greaterThen(Integer id) {
        List<Weather> list = weathers.values()
                .stream()
                .filter(temp -> temp.getTemperature() > id)
